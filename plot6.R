...
# only Baltimore City and Los Angeles County, so we'll need to filter the main list to the correct codes, 
# then aggregate by year.

mv_source_emissions <- emissions_data[emissions_data$fips %in% c('24510', '06037') & emissions_data$SCC %in% mv_sources$SCC, ]

mv_source_emissions_by_year <- aggregate(mv_source_emissions$Emissions,
                                         by = list(year = mv_source_emissions$year, fips = mv_source_emissions$fips),
                                         sum)
...
total_bars <- ggplot(mv_source_emissions_by_year, 
       aes(x = year_factor, y = x, color = fips, fill = fips)
) + 
...
mv_source_emissions_by_year$delta_2009 <- apply(mv_source_emissions_by_year, 1, function(row){ 
  v1999 <- mv_source_emissions_by_year[mv_source_emissions_by_year$fips == row["fips"] & 
                                         mv_source_emissions_by_year$year == min(mv_source_emissions_by_year$year), c('x')]
...
delta_bars <- ggplot(mv_source_emissions_by_year, 
                      aes(x = year_factor, y = delta_2009, group = fips, fill = fips)
) + 
  geom_bar(stat = 'identity') +
  ...
  geom_hline(aes(yintercept = 0, color = fips),
             linetype = 'dashed') +
  facet_wrap(~ fips, ncol = 1) +
...
delta_lines <- ggplot(mv_source_emissions_by_year, 
       aes(x = year_factor, y = delta_2009_pct, group = fips, color = fips)
) + 
  geom_line() +
  ggtitle('Change in Motor-Vehicle\nPM2.5 Emissions') +
  xlab('Year') +
